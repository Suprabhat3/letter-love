...
import { readReactionCounts } from "@/lib/reactions";
...
export default function DashboardPage() {
  ...
  // The number that brings people back. Owner views are excluded server-side,
  // so this counts real recipients rather than your own previews.
  const totalOpens = cards.reduce((sum, c) => sum + (c.view_count ?? 0), 0);
  ...
  if (authLoading || !user) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-linear-to-br from-pink-50 via-rose-50 to-purple-50 dark:bg-background">
        <motion.div
          animate={{ rotate: 360 }}
  ...
  return (
    <main className="min-h-screen relative bg-linear-to-br from-pink-50 via-rose-50 to-purple-50 dark:bg-background transition-colors duration-500">
      <ShareModal
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className="absolute -top-[30%] -right-[15%] w-250 h-250 rounded-full bg-linear-to-br from-pink-300/30 to-purple-300/30 dark:from-pink-900/20 dark:to-purple-900/20 blur-[120px]"
        />
        <motion.div
          animate={{
            scale: [1, 1.3, 1],
          ...
          className="absolute -bottom-[30%] -left-[15%] w-200 h-200 rounded-full bg-linear-to-tr from-blue-300/30 to-teal-300/30 dark:from-blue-900/20 dark:to-teal-900/20 blur-[120px]"
        />
        <motion.div
          animate={{
            scale: [1, 1.1, 1],
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-150 h-150 rounded-full bg-linear-to-br from-rose-300/20 to-orange-300/20 dark:from-rose-900/10 dark:to-orange-900/10 blur-[100px]"
        />
      </div>
          ...
          >
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="px-4 py-1.5 rounded-full bg-linear-to-r from-pink-500/10 to-purple-500/10 border border-pink-200/50 dark:border-pink-500/20 text-pink-600 dark:text-pink-300 text-sm font-semibold tracking-wide backdrop-blur-sm">
                  Your Creative Space
                </span>
              </div>
              <h1 className="text-5xl sm:text-6xl lg:text-7xl font-serif font-bold text-foreground tracking-tight">
                Hello,{" "}
                <span className="text-transparent bg-clip-text bg-linear-to-r from-pink-500 via-rose-500 to-purple-600">
            ...
            >
              ...
              >
                <motion.div
                  className="absolute inset-0 bg-linear-to-r from-pink-400 to-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"
                  initial={false}
                />
        ...
        {loading ? (
          ...
          >
            <motion.div
              ...
              className="w-32 h-32 bg-linear-to-br from-pink-100 to-purple-100 dark:from-pink-900/30 dark:to-purple-900/30 rounded-full flex items-center justify-center mb-8 shadow-lg"
            >
...
function StatCard({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-linear-to-br opacity-0 group-hover:opacity-100 transition-opacity rounded-3xl blur-xl -z-10"
        style={{
      ...
      <div className="relative bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-3xl p-6 shadow-lg hover:shadow-xl transition-all overflow-hidden">
        <div
          className="absolute top-0 right-0 w-32 h-32 bg-linear-to-br opacity-10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2"
          style={{
            backgroundImage: `linear-gradient(to bottom right, var(--tw-gradient-stops))`,
          }}
        />
        ...
        <div
          className={`inline-flex p-3 rounded-2xl bg-linear-to-br ${gradient} mb-4 shadow-lg`}
        >
        ...
        <motion.div
          className="absolute bottom-0 left-0 h-1 bg-linear-to-r"
          style={{
...
function LetterCard({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white/80 dark:bg-card/80 backdrop-blur-sm border border-black/5 dark:border-white/10 rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-pink-200/30 dark:hover:shadow-pink-900/20 hover:-translate-y-2 transition-all duration-500 flex flex-col min-h-95"
    >
      {/* Gradient Overlay on Hover */}
      <motion.div
        className="absolute inset-0 bg-linear-to-br from-pink-500/5 via-purple-500/5 to-rose-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        initial={false}
      />

      {/* Card Preview Header */}
      <div
        className="h-32 sm:h-36 p-4 sm:p-6 flex flex-col justify-between relative overflow-hidden shrink-0"
        style={{
          background: template
      ...
      <div className="p-4 sm:p-6 flex flex-col flex-1 relative z-10">
        ...
        <div className="pt-3 sm:pt-4 border-t border-black/5 dark:border-white/5 flex items-center justify-between text-xs shrink-0">
...
/**
 * Whether the letter has actually been read, and what came back.
 *
 * This replaces a pulsing green dot that meant nothing at all — it animated on
 * every card whether or not anyone had ever opened it. A status indicator that
 * is always green is worse than none, because it teaches the sender to ignore
 * the one place real information will appear.
 *
 * The counts arrive on the card row itself (migration 0003 denormalizes them),
 * so this costs no extra query. Owner views are excluded server-side, which is
 * what makes "Not opened yet" trustworthy even after you have previewed your
 * own card a dozen times.
 */
function CardReceipt({ card }: { card: SharedCard }) {
  const opens = card.view_count ?? 0;
  const people = card.unique_view_count ?? 0;
  const replies = card.reply_count ?? 0;
  const reactions = readReactionCounts(card.reaction_counts);
  const reacted = Object.entries(reactions).filter(([, n]) => (n ?? 0) > 0);
  const recipient = readCardContent(card.data).recipientName || "They";

  if (opens === 0) {
    const url = `${typeof window !== "undefined" ? window.location.origin : ""}/share/${card.id}`;
    const nudge = encodeURIComponent(
      `Maine tumhare liye kuch likha hai 💌 ${url}`,
    );

    return (
      <div className="mb-3 flex items-center justify-between gap-2 rounded-xl bg-black/3 dark:bg-white/5 px-3 py-2">
        <span className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className="h-2 w-2 rounded-full bg-muted-foreground/30" />
          Not opened yet
        </span>
        {/* WhatsApp, because that is where the link was sent in the first
            place. wa.me with no number opens the contact picker. */}
        <a
          href={`https://wa.me/?text=${nudge}`}
          target="_blank"
          rel="noopener noreferrer"
          className="shrink-0 rounded-lg bg-green-500/10 px-2.5 py-1 text-[11px] font-semibold text-green-700 dark:text-green-400 transition-colors hover:bg-green-500/20"
        >
          Remind
        </a>
      </div>
  ...
  return (
    <div className="mb-3 flex flex-col gap-1.5 rounded-xl bg-green-500/5 px-3 py-2">
      <span className="flex items-center gap-2 text-xs font-medium text-green-700 dark:text-green-400">
        <span className="h-2 w-2 rounded-full bg-green-500 shadow-sm shadow-green-500/50" />
        {recipient} opened {opens === 1 ? "it once" : `${opens} times`}
        {people > 1 && ` · ${people} people`}
        {card.last_viewed_at && ` · ${timeAgo(card.last_viewed_at)}`}
      </span>

      {(reacted.length > 0 || replies > 0) && (
